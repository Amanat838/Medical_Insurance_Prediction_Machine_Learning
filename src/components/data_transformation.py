...
class DataTransformation:
    ...
    def get_data_transformer_object(self):
        ...
        try:
            ...
            logging.info(f"Numerical: {numerical_columns}")

            # Combine both numerical and categorical pipelines into a single preprocessor
            preprocessor = ColumnTransformer(
                [
                    # Mean we are doing pipeline of these steps to given column name and same for categorical columns
    ...
    def initiate_data_transformation(self, train_path, test_path):
        try:
            ...
            # We are predicting math score, so this is our target column
            target_column_name = 'charges'
            # axis=1 means we are dropping column
